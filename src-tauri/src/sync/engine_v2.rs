...
use super::engine::{SyncConfig, SyncProgress, SyncStats, TierComplete};
use super::media::MediaDownloader;
use super::pool::RelayPool;
use super::processing;
use super::pruning;
use super::threads::ThreadContext;
use super::types::{EventSource, SyncPhase, WOT_CRAWL_FREQUENCY};

/// SyncEngine v2 — relay-centric outbox model with NIP-65 routing.
///
/// Same public interface as SyncEngine (new, start, sync_tier, sync_stats)
/// so lib.rs can swap between v1 and v2 via config.
pub struct SyncEngineV2 {
    graph: Arc<WotGraph>,
    db: Arc<Database>,
...
impl SyncEngineV2 {
    pub fn new(
        ...
        storage_media_gb: f64,
        sync_config: SyncConfig,
        _max_event_age_days: u32, // Kept for API compat; v2 uses tiered retention
    ) -> Self {
        ...
        let final_relays = if valid_relays.is_empty() {
            warn!("SyncEngineV2: no valid relays, using defaults");
            vec![
        ...
        info!(
            "SyncEngineV2: initialized with {} relays: {:?}",
            final_relays.len(),
            final_relays
        );
    ...
    async fn run(&self) -> Result<()> {
        ...
        loop {
            ...
            cycle += 1;
            info!("SyncEngineV2: starting cycle {}", cycle);
            ...
            info!("SyncEngineV2: cycle {} complete", cycle);
