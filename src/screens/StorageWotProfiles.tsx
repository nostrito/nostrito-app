...
export const StorageWotProfiles: React.FC = () => {
  ...
  const loadNotes = useCallback(async (until?: number) => {
    ...
    try {
      ...
      console.error("[storage:wot] load notes failed:", e);
    }
  ...
  // Load media — scans events for all media references
  useEffect(() => {
    if (tab === "media" && allMedia.length === 0) {
      setMediaLoading(true);
      invoke<EventMediaRef[]>("get_event_media_for_category", { category: "wot", limit: 500 })
        .then(setAllMedia)
        .catch(e => console.error("[storage:wot] load media failed:", e))
        ...
        <div className="storage-notes-list">
          ...
          {!notesLoading && notes.length === 0 && (
            <EmptyState
              ...
              message="No notes from WoT profiles yet."
              hint="Notes from your web of trust will appear here as they sync."
            />
          )}
          {notes.map(event => (
            <NoteCard
          ...
          <div className="storage-media-grid">
            ...
            {!mediaLoading && filteredMedia.length === 0 && (
              <EmptyState
                ...
                message="No cached media yet."
                hint="Media from WoT profiles will appear here as it downloads."
              />
            )}
